 ...
 * @dev Functions are intended to be called in the context of a Vault via delegateCall, except indicated otherwise
 */
interface ILendingProvider {
   ...
   * @param keyAsset address of the inquiry operator
   ...
   * @dev Provider implementation may or may not require all 3 inputs
   */
  ...
  /**
   * @notice Performs borrow operation at lendign provider on behalf vault
   ...
   * E.g 8.5% APR = 0.085 x 1e27 = 85000000000000000000000000
   * - Must not require Vault context
   */
  function getDepositRateFor(IVault vault) external view returns (uint256 rate);

  /**
   * @notice Reutnrs the latest BORROW annual percent rate (APR) at lending provider
   * @param vault IVault required by some specific providers with multi-markets, otherwise pass address(0)
   * @dev Requirements:
   * - Must return the rate in ray units (1e27)
   * E.g 8.5% APR = 0.085 x 1e27 = 85000000000000000000000000
