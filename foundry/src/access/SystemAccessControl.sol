 ...
 * @notice Abstract contract that shoul be inherited by contract implementation that
...
contract SystemAccessControl is CoreRoles {
  ...
  /**
   * @dev Modifier that checks `msg.sender` is the defined `timelock`
   * in {Chief} contract
   */
  ...
  /**
   * @notice Init of a new {SystemAccessControl} contract
