...
// 3. Create a registry entry with addresses for required resources
...
contract ForkingSetup is CoreRoles, Test, ChainlinkFeeds {
  ...
  function setUpFork(uint32 domain) public {
    ...
    if (reg.connext == address(0) && reg.weth == address(0) && reg.usdc == address(0)) {
      revert("No registry for this domain");
    }
    ...
    if (domain == GOERLI_DOMAIN || domain == OPTIMISM_GOERLI_DOMAIN || domain == MUMBAI_DOMAIN) {
      ...
      // WETH assumed 2k usd/eth, dai = 1usd
      mockOracle.setUSDPriceOf(collateralAsset, 2000e8); // 2000 usd
      mockOracle.setUSDPriceOf(debtAsset, 1e8); // 1 usd
      oracle = mockOracle;
    }
  }
    ...
    internal
  {
    ...
    vm.label(collateralAsset, collateralAssetName);
  ...
  function _setActiveProdiers(IVault v, ILendingProvider provider) internal {
  ...
  /**
   * @dev This function was created to avoid the "Stack too deep" error in `_getDepositAndBorrowCallData`
   */
