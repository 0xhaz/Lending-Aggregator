...
contract MockingSetup is CoreRoles, Test {
    ...
    uint256 public constant USD_PER_DAI_PRUICE = 1e8;
    ...
    constructor() {
        ...
        // WETH and DAI prices: 2000 DAI per ETH
        oracle.setUSDPriceOf(collateralAsset, USD_PER_ETH_PRICE);
        oracle.setUSDPriceOf(debtAsset, USD_PER_DAI_PRUICE);
        ...
        // Initializa with a default mockProvider
        ...
        bytes memory executionCall = abi.encodeWithSelector(chief.setVaultStatus.selector, address(vault), true);
    ...
    // those that needs more than one signature in the same tx
    function _getPermitBorrowArgs(LibSigUtils.Permit memory permit, uint256 ownerPrivateKey, address vault_)
        internal
        view
    ...
    //   plusNonce is necessary for compound operations
    // those that needs more than one signature in the same tx
    function _getPermitWithdrawArgs(LibSigUtils.Permit memory permit, uint256 ownerPrivateKey, address vault_)
        internal
        view
        returns (uint256 deadline, uint8 v, bytes32 r, bytes32 s)
    {
