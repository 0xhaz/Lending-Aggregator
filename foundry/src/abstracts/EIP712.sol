...
/**
 * @title EIP712
 * @notice EIP712 abstract contract for VaultPermission
 * @dev https://eips.ethereum.org/EIPS/eip-712 is a standard for hashing
 * and signing of structured data
 ...
 * The main difference with OZ is that the "chainid" is not included in the
 * domain separator but in the structHash. The rationale behind is to adapt EIP712 to our cross-chain message
 * signing: allowing a user on chain A to sign a message that will be verified on chain B
 * If we were to include the "chainid" in the domain separator, that would require the user to switch networks back
 * and forth, because of the limitation: "The user-agent should refuse signing if it does not match the currently
 * active chain". That would seriously deteriorate the user experience.
 *
 * Indeed, EIP712 doesn't forbid it as it states that "Protocol designers only need to include the fields that make sense for
 * their signing domain". into the the struct "EIP712Domain". However, we decided to add a ref to "chainid" in the param salt.
 * Togehter with chainid in the typeHash, we assume those provide sufficient security guarantees.
 */
...
abstract contract EIP712 {
  ...
  function __EIP712_initialize(string memory name_, string memory version_) internal {
    ...
    _CACHED_DOMAIN_SEPARATOR = _buildDomainSeparator(hashedName, hashedVersion, typeHash);
  ...
  function _domainSeparatorV4() internal view returns (bytes32) {
    if (address(this) == _CACHED_THIS && block.chainid == _CACHED_CHAIN_ID) {
      ...
      return _buildDomainSeparator(_HASHED_NAME, _HASHED_VERSION, _TYPE_HASH);
    }
  }

  /**
   * @dev Builds and returns domain separator according to inputs
   * @param typeHash cached in this contract
   * @param nameHash hashed name of the signing domain
   * @param versionHash hashed version of the signing domain
   */
