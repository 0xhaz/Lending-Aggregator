...
/**
 * @title ChainlinkComputeFeed
 *
 * @notice Contract that combines two  Chainlink price feeds into one
 * resulting feed denominated in another currency asset
 *
 * @dev For example: [wsteth/eth] - feed and [eth/usd] - feed to return a [wsteth/usd] - feed
 * Note: Ensure units work, this contract multiplies the two feeds to get the final price
 */
...
contract ChainlinkComputedFeed {
  ...
  function decimanls() external view returns (uint8) {
    return _decimals;
  }
    ...
    returns (ChainlinkResponse memory clFeed, ChainlinkResponse memory clInter)
  {
    ...
    // Perform checks to the returned chainlink response
