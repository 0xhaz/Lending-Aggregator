...
contract FakeBorrowingVault is BorrowingVault {
  ...
  constructor(
    ...
    BorrowingVault(asset_, debtAsset_, oracle_, chief_, name_, symbol, providers_, maxLtv_, liqRatio_)
  {
    _attacker = attacker_;
  }
...
contract SimpleRouterUnitTest is MockingSetup, MockRoutines {
  ...
  function test_Payback_And_Withdraw() public {
  ...
  function test_Deposit_ETH_And_Borrow() public {
  ...
  function test_Deposit_ETH_And_Withdraw_ETH() public {
  ...
  function test_Sweep_ETH(uint256 amount_) public {
  ...
  function test_Try_Foe_Sweep_ETH(address foe, uint256 amount_) public {
  ...
  function test_Deposit_Approval_Attack() public {
    _dealMockERC20(collateralAsset, ALICE, amount);

    // Alice has approved for some reason the router to spend her tokens
    vm.prank(ALICE);
    ...
    actions[0] = IRouter.Action.Deposit;
    // Attacker sets themselves as the receiver
  ...
  function test_Infinite_Approval_With_Fake_Vault() public {
    address attacker = BOB;

    ILendingProvider[] memory providers = new ILendingProvider[](1);
    providers[0] = new MockProvider();
    ...
    bytes[] memory args = new bytes[](1);
    actions[0] = IRouter.Action.Deposit;
    args[0] = abi.encode(address(fakeVault), amount, attacker, ALICE);
  ...
  function test_Withdrawal_Approval_Attack() public {
    _dealMockERC20(collateralAsset, ALICE, amount);
    ...
    // Alice approves withdrawal allowance for the router to spend her tokens
    ...
    // Using Alice `withdrawAllowance` and attempts to deposit
    // with themselves as the receiver
  ...
  function test_Withdrawal_With_Permit_Attack() public {
    _dealMockERC20(collateralAsset, ALICE, amount);
    ...
    // Asset attacker received ETH balance from attack attempt
    assertEq(BOB.balance, 0);
  }

  function test_Borrow_With_Permit_Attack() public {
    ...
    // Asset attacker received no shares from attack attempt
    assertEq(newVault.balanceOf(BOB), 0);
  }

  function test_Deposit_Stuck_Funds_Exploit() public {
    // Funds are stuck in the router
    _dealMockERC20(collateralAsset, address(simpleRouter), amount);

    // attacker attempts to deposit the stuck funds
    IRouter.Action[] memory actions = new IRouter.Action[](1);
    bytes[] memory args = new bytes[](1);
  ...
  function test_Router_Self_Destruct_DOS_Attack() public {
    SelfDesctructor destroyer = new SelfDesctructor();

    vm.deal(address(this), 1 wei);
    destroyer.attack{value: 1 wei}(payable(address(simpleRouter)));

    // Alice should be able to interact with the router as normal
    _depositAndBorrow(amount, borrowAmount, vault);
  ...
  function test_Attack_Via_Swapper() public {
    ...
    attackerActions[1] = IRouter.Action.Withdraw;
    attackerArgs[1] = abi.encode(address(vault), amount, address(simpleRouter), ALICE);
    attackerActions[2] = IRouter.Action.Swap;
    attackerArgs[2] = abi.encode(
      address(swapper),
      collateralAsset,
      address(tWBTC),
      amount,
      ...
      ATTACKER,
      ATTACKER,
      0 // not handling slippage in this test
    );
  ...
  function test_Withdraw_Soft_Max(uint128 withdrawAmount) public {
    uint256 depositAmount = 0.1 ether;
    vm.assume(withdrawAmount > depositAmount);
    {
      // Setup of WithdrawETH
      deal(collateralAsset, depositAmount);
    }
    do_deposit(depositAmount, vault, ALICE);
    ...
    bytes[] memory args = new bytes[](3);
    {
      ...
      // Replace permit actions arguments, now with signature values
      args[0] =
