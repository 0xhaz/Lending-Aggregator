...
/**
 * @title BaseVault
 * @notice Abstract contract that defines the basic common functions
 * and interface for all vault types. User state is kept in vaults via tokenized
 * shares compliant to ERC4626.
 * BaseVault defines but does not implement the debt handling functions. Slippage
 * protected functions are available through ERC5143 extension. The `_providers`
 * of this vault are the liquidity source for lending, borrowing and/or yielding
 * operations
 * Setter functions are controlled by timelock, and roles defined in
 * {SystemAccessControl}. Pausability in core functions is implemented for
 * emergency cases. Allowance and approvals for value extracting operations is
 * possible via signed messages defined in {VaultPermissions}
 * A rebalancing function is implemented to move vault's fund across providers
 */
...
abstract contract BaseVault is ERC20, SystemAccessControl, PausableVault, VaultPermissions, IVault {
   ...
   * @dev Requirements:
   * - Must assign `asset_` {ERC20-decimals} and `decimals` equal
    ...
    ERC20(name_, symbol_)
  {
    ...
    /// @dev pause all actions that will be unpaused when initalizing the vault
  ...
  /**
   * @notice Slippage protected `deposit()` per EIP-5143
   * @param assets amount to deposit
   * @param receiver to whom `assets` amount is credited
   * @param minShares amount expected from this deposit
   *
    ...
    returns (uint256)
  {
    ...
    if (receivedShares < minShares) revert BaseVault__deposit_slippageTooHigh();
    return receivedShares;
  }

  /**
   * @notice Slippage protected `mint()` per EIP-5143
    ...
    returns (uint256)
  {
    ...
    if (pulledAssets > maxAssets) revert BaseVault__mint_slippageTooHigh();
    return pulledAssets;
  }

  /**
   * @notice Slippage protected `withdraw()` per EIP-5143
   * @param assets amount to withdraw
   * @param receiver to whom `assets` amount is credited
   * @param owner to whom `shares` amount is debited
   * @param maxShares amount that must be debited when calling `withdraw()`
   * @dev Refer to https://eips.ethereum.org/EIPS/eip-5143
   * If needed to withdraw the maximum amount it is recommended
   * to use non-EIP5143 method
   * Requirements:
    ...
    returns (uint256)
  {
    ...
    if (burnedShares > maxShares) revert BaseVault__withdraw_slippageTooHigh();
    return burnedShares;
  }
    ...
    returns (uint256)
  {
    ...
    if (receivedAssets < minAssets) revert BaseVault__redeem_slippageTooHigh();
  ...
  /**
   * @notice Returns the shares amount allowed to transfer from `owner`
   * to `receiver`
   * @param owner of the shares
   ...
   * @param receiver to whom share allowance is being granted
   * @param shares amount of allowance to grant
   *
   * @dev Recommend to use increase/decrease WithdrawAllowance functions
   ...
   * @param assets amount to deposit
   * @param shares amount to mint for `receiver`
   */
   ...
   * `assets_` and `shares_` to withdraw if passed amounts exceed `owner's` debtShares/debtBalance
   * Requirements:
   * - Must revert for all conditions not passed
   ...
   * @param caller or {msg.sender} of the function
  ...
  /**
   * @dev Function to handle common flaw for `withdraw()` and `redeem()`
   * It returns the updated `assets_` and `shares_` values if applicable
   *
   * @param assets amount transferred during this withdraw
  ...
  {
    /**
     * @dev If passed `assets` argument is greater than the max amount `owner` can
     * withdraw. The maximum withdrawable will be withdrawn and returned from
     * `withdrawChecks()`
     */
   ...
   * Requirements:
   * - Must return zero if `assets` or `totalSupply()` is zero
  ...
  /**
   * @dev Conversions function from `shares` to asset type with support for rounding direction
   * Requirements:
   * - Must return zero if `totalSupply()` is zero
   ...
   * @param owner of the shares
   * @param spender that can spend the shares
   ...
   * @param owner of the debt
   * @param operator who can process borrow allowance on behalf of `owner`
   ...
   * @param owner address who signed the permit
   * @param receiver who can spend borrow allowance
   * @param value amount of borrow allowance
   * @param deadline timestamp at when the permit expires
   * @param actionArgsHash keccak256 of the abi.encoded(args,actions) to be perfomed in {BaseRouter._internalBundle}
   ...
   * @param provider to call the method
   */
    ...
    internal
  {
    ...
    address(provider).functionDelegateCall(
      data, string(abi.encodePacked(name, ": delegatecall failed"))
    );
  }

  /**
   * @dev Returns balance of `assets` or `debtAsset` of this vault at
   * all listed providers in `_providers` array
   *
   * @param method string of the method to call: "getDepositBalance" or "getBorrowBalance"
   */
  function _checkProvidersBalance(string memory method) internal view returns (uint256 assets) {
    ...
    for (uint256 i; i < len;) {
      returnedBytes = address(_providers[i]).functionStaticCall(
        callData, "checkProvidersBalance: balance call failed"
      );
  ...
  /**
   * @dev Set the array of providers of this vault
   * Requirements:
   * - Must be implemented at {BorrowingVault} or {YieldVault}
   * - Must infinite approve ERC20 transfer of `asset` or `debtAsset` to all providers
   * - Must emit ProvidersChanged event
   * @param providers array of providers to set
   */
  function _setProviders(ILendingProvider[] memory providers) internal virtual;

  /**
   * @dev Set the active provider of this vault
   * Requirements:
   * - Must emit ActiveProviderChanged event
   ...
   * @param amount being rebalanced to check agains
   */
