...
contract MathPrecision {
  ...
  uint256 internal constant PRECISION = 1e27; // 27 decimal places
...
contract MathPrecisionTest is Test {
  ...
  function test_Math_Precision_Unchanged_Equivalence_Fuzz(uint128 numA, uint128 sharesA) public {
    // uint128 numA = 998567;
  ...
  function test_Math_Precision_Check_Conversion_And_Remainder_Fuzz(
    uint128 numA,
    uint128 sharesA,
    uint128 shareLimit
  )
    public
  {
    ...
    if (shareLimit < sharesA) {
      ...
      */
      ...
      assertLt(errorDiff, ERROR_LIMIT);
    } else {
      assertEq(computedSharesA, sharesA);
      assertEq(computedA, numA);
