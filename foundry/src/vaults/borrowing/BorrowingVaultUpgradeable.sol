...
/**
 * @title BorrowingVaultUpgradeable
 * @notice Upgradeable implmentation of {BorrowingVault}
 */
...
contract BorrowingVaultUpgradeable is BaseVaultUpgradeable {
   ...
   * @param debtPaid `owner's` amount of debt paid during liquidation
   * @param price of collateral at the time of liquidation
   * @param liquidationFactor what % of debt is being liquidated
   */
  event Liquidate(
  ...
  uint256 public constant MAX_LIQUIDATION_CLOSE_FACTOR = PRECISION_CONSTANT; // 100%

  /// @notice Returns halth factor threshold at which max liquidation can occur
  uint256 public constant FULL_LIQUIDATION_THRESHOLD = 95e16; // 95%

  /// @notice Returns the penalty factor at which collateral is sold during liqudiation: 90% below oracle price
  ...
  /////////////////////////////// INITIALIZE FUNCTION ///////////////////////////////
  /**
   * @notice Initializes a new {BorrowingVault}
   *
   ...
   * @param providers_ array of lending providers that can be used in this vault
   *
   * @dev Requirements:
   * - Must be initialized with a set of valid lending providers
  ...
  function _beforeTokenTransfer(address from, address to, uint256 amount) internal view override {
    /**
     * @dev Hook check activated only when called by OZ {ERC20-transfer}
  ...
  function increaseBorrowAllowance(
    ...
    returns (bool)
  {
    return VaultPermissions.increaseBorrowAllowance(operator, receiver, amount);
  }
  ...
  function decreaseBorrowAllowance(
    ...
    returns (bool)
  {
    return VaultPermissions.decreaseBorrowAllowance(operator, receiver, amount);
  }
   ...
   * (collateral) balance and the current 'asset' price
   * Requirements:
   ...
   * - Must not be implemented in {YieldVault} contract
   * - Must read price from {FujiOracle}
   *
  ...
  function _computeFreeAssets(address owner) internal view returns (uint256 freeAssets) {
    ...
    if (debtShares == 0) {
      ...
      if (lockedAssets == 0) {
        // Handle wei level amonuts in where 'lockedAssets' < 1 wei
        lockedAssets = 1;
      }
   ...
   * - Must return `debt` if `debtSharesSupply` is 0
   *
   ...
   * Requirements:
   * - Must return zero if `debtSharesSupply` is 0
   *
   ...
   * @param owner to whom `debtShares` gets minted
    ...
    returns (uint256 debt_, uint256 shares_)
  {
    ...
    if (remainder > 0) {
      /**
       * @dev Since the `_payback` only pulls (ERC20) that is needed to pay back maxAmount,
       * this logic handles excess amount `remainder` by pulling from `msg.sender` and
       * returning to the `owner` the `remainder`
       */
   ...
   *
   * @param caller msg.sender
   * @param owner to whom `debtShares` gets burned
   * @param assets amount of debt
   * @param shares amount of debtShares
   */
  function _payback(
   ...
   * - Must revert for all conditions not passed
   *
   * @param owner of the debt acountability
   * @param debt or borrowed amount of debt asset
   ...
   * @param amount of shares to mint
   */
   ...
   * @param amount of shares to burn
   */
   ...
   *
   * @dev Requirements:
   * - Must not be called by the zero address
  ...
  function setOracle(IFujiOracle newOracle) external onlyTimelock {
    ...
    oracle = newOracle;
    emit OracleChanged(address(newOracle));
  }
