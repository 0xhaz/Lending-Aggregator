...
/**
 * @title PausableVault
 * @notice Abstrace pausable contract developed for granular control over vault
 * actions. This contract should be inherited by a vault implementation. The
 * code is inspired on OpenZeppelin's Pausable contract.
 */
...
abstract contract PausableVault is IPausableVault {
  ...
  /**
   * @dev Modifier to make a function callable only when 'VaultAction' in the
  ...
  /**
   * @dev Forces set paused state for all `VaultAction`
   */
  ...
  /**
   * @dev Forces set unpause state for all `VaultAction`
   */
