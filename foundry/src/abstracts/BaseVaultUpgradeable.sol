 ...
 * @notice Upgradable base contract for {BaseVault}
 */
  ...
  IVaultUpgradeable
{
   ...
   * - Must be override to call {VaultPermissions-_setWithdrawAllowance}
   * - Must convert `shares` into `assets` amount before calling internal function
   */
  function approve(
    address receiver,
    uint256 shares
   ...
   * Requirements:
   * - Must return zero if `assets` to `totalSupply()` == 0
  ...
  /**
   * @dev Conversion function from `shares` to assets equivalent with support for rounding direction
   * Requirements:
   * - Must return zero if `suppply` to `totalSupply()` == 0
   *
  ...
  /**
   * @dev Function to handle common flow for `withdraw()` and `redeem()` actions
   * It returns the updated `assets` and `shares` after the action is performed
   *
   * @param assets amount transferred during this withdraw
   * @param shares amount burned to `owner` during this withdraw
   * @param caller of msg.sender
   * @param receiver to whom `assets` are credited by `shares` amount
   * @param owner of the `shares` being burned
   */
  ...
  /**
   * @dev Runs common checks for all "withdraw" or "redeem" actions in this vault and returns
   * maximum `assets_` and `shares_` to withdraw if passed amounts exceed `owner's` debtShares/debt balance
   * Requirements:
   * - Must revert for all conditions not passed
   *
