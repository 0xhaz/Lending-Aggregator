 ...
 * @notice Contract that facilitates rebalancing of funds between providers
 */
...
contract RebalancerManager is IRebalancerManager, SystemAccessControl {
   ...
   * @param amount to be checked and rebalanced
   * @param from provider address
   */
  function _checkAssetsAmount(IVault vault, uint256 amount, ILendingProvider from) internal view {
    uint256 assetsProvider = from.getDepositBalance(address(vault), vault);
    if (amount > assetsProvider) {
   ...
   * @param assets amount to be rebalanced
   * @param debt amount to be rebalanced
   * @param from provider where `assets` and `debt` are currently
   * @param to provider where `assets` and `debt` will be moved
   */
  ...
  /**
   * @dev Initiates a flashloan for a rebalancing operation
   * @param vault being rebalanced
   * @param assets amount to be rebalanced
   * @param debt amount to be rebalanced
   * @param from provider where `assets` and `debt` are currently
  ...
  /**
   * @dev Checks this address is the flashloan originator. This check applies to
   * a {BorrowingVault} only
   * @param vault being rebalanced
   * @param assets amount to be rebalanced
   * @param debt amount to be rebalanced
   * @param from provider where `assets` and `debt` are currently
