 ...
 * User start is kept at vaults via token-shares complian to ERC4626, including
 * extensions for debt asset and their equivalent debtshares
 * Debt shares are not transferable.
 * Slippage protexted functions include `borrow()` and `payback()`
 ...
 * function to determine user's health factor
 */
...
contract BorrowingVault is BaseVault {
   ...
   * @param caller The caller of the liquidation function
   ...
   * @param price price of collateral at which liquidation was executed
  ...
  /// @notice Returns default liquidation closr factor: 50% of debt
  ...
  uint256 public constant FULL_LIQUIDATION_THRESHOLD = 95e16; // 95%

  /// @notice Returns the penalty factor at which collateral is sold during liquidation: 90% below oracle price
  uint256 public constant LIQUIDATION_PENALTY = 0.9e18; // 90%
   ...
   * @param maxLtv_ initially set in this vault
   * @param liqRatio_ initially set in this vault
   * @dev Requirements:
   * - Must be initialized with a set of providers
   * - Must set first provider in `providers_` array as `activeProvider`
   * - Must initialize `maxLtv` and `liqRatio` with non-zero values
   * - Must check `maxLtv` < `liqRatio`
   * - Must check `debtAsset_` erc20-decimals and `_debtDecimals` are equal
   */
  constructor(
    address asset_,
    address debtAsset_,
    ...
    returns (uint256)
  {
    ...
    if (receivedDebtShares > maxDebtShares) revert BorrowingVault__borrow_slippageTooHigh();
   ...
   * @param shares amount of debtShares to mint
   * @param receiver address to whom  borrowed amount will be transferred
   * @param owner address who will incur the debt
   * @param minDebt amount that must be sent to receiver in this mintDebt call
   * @dev Requirements:
    ...
    returns (uint256)
  {
    ...
    if (receivedDebt < minDebt) revert BorrowingVault__mintDebt_slippageTooHigh();
    return receivedDebt;
  }
   ...
   * @param debt amount of debt to payback
   * @param owner address who will incur the debt
   ...
   * @param shares amount of debtShares to burn
   * @param owner address who will incur the debt
  ...
  function burnDebt(uint256 shares, address owner, uint256 maxAmount) public returns (uint256) {
    ...
    if (maxPulledAmount > maxAmount) revert BorrowingVault__burnDebt_slippageTooHigh();
  ...
  {
    if (receiver == address(0)) revert BorrowingVault__liquidate_invalidInput();

    address caller = msg.sender;

    uint256 liquidationFactor = getLiquidationFactor(owner);
    if (liquidationFactor == 0) revert BorrowingVault__liquidate_positionHealthy();

    if (liqCloseFactor_ > liquidationFactor) revert BorrowingVault__liquidate_moreThanAllowed();
    ...
    // Internal share adjustment between `owner` and `liquidator`
  ...
  /**
   * @notice Sets `newOracle` address as the {FujiOracle} of this vault
   * @param newOracle address of the new oracle
   * @dev Requirements:
   * - Must not be a zero address
   * - Must emit an OracleChanged event
   * - Must be called from a timelock
   */
  function setOracle(IFujiOracle newOracle) external onlyTimelock {
    if (address(newOracle) == address(0)) revert BaseVault__setter_invalidInput();
    oracle = newOracle;
  ...
  /**
   * @notice Sets the liquidation ratio factor of this vault
   * @param liqRatio_ factor to set
   * @dev Restrictions:
   * - Must be called from a timelock
  ...
  /////////////////// ASSET MANAGEMENT: OVERRIDE IERC4626 /////////////////////
   ...
   * @param from address from whom shares are being transferred
   * @param to address to whom shares are being transferred
   * @param amount of shares being transferred
   */
  function _beforeTokenTransfer(address from, address to, uint256 amount) internal view override {
    /**
     * @dev Hook check activated only when called by OZ {ERC20-_transfer}
     * User must not be able to transfer asset-shares if locked as collateral
     */
   ...
   * - Must not be implemented in a {YieldVault}
   * - Must read price from oracle
   ...
   *  (Corresponds to a case where you divide by 0)
   * - Must return `debt` if debtSharesSupply = 0
   * @param debt amount of debt to convert to debtShares
   ...
   * Requirements:
   * - Must return 0 if debtSharesSupply = 0
   *
   * @param shares amount of debtShares to convert to debt
   * @param rounding direction of division remainder
   */
  ...
  /**
   * @dev Computes max borrow amount a user can take given their `assets`
   * (collateral) balance and price
   * Requirements:
   * - Must read price from oracle
   *
   * @param borrower address to whom max borrow is being checked
   */
  ...
  /**
   * @dev Perform borrow action at provider. Borrow / mintDebtShares common workflow
   * Requirements:
   * - Must call `activeProvider` in `_executeProviderAction()`
   * - Must emit a Borrow event
   * @param caller or operator
   * @param receiver to whom borrowed amount will be transferred
   * @param owner to whom `debtShares` will be minted
   ...
   * It returns the updated `debt` and `shares` value if applicable
   * @param debt borrowed amount of debt asset
   * @param shares corresponding to debtShares
   * @param owner to whom `debtShares` will be burned
   * @param caller msg.sender in this context
   */
    ...
    returns (uint256 debt_, uint256 shares_)
  {
    ...
    if (remainder > 0) {
      /**
       * @dev Since the `payback()` only pulls ERC20 that is needed to payback
       * maxAmount, this logic handles excess amonut `remainder` by pulling from
       * `msg.sender` and returning to the `owner` the `remainder` amount
       */
   ...
   * The `remainder` will be non-zero if the passed `shares` args exceeds
   * the debtShares balance of `owner`
   * Requirements:
   * - Must revert for all conditions not passed
   *
   * @param owner of the debt accountability
  ...
  function _burnDebtShares(address owner, uint256 amount) internal {
    ...
    if (balance < amount) revert BorrowingVault__burnDebtShares_amountExceedsBalance();
    unchecked {
  ...
  function _setProviders(ILendingProvider[] memory providers) internal override {
    ...
    for (uint256 i; i < len;) {
      if (address(providers[i]) == address(0)) revert BaseVault__setter_invalidInput();
      _asset.forceApprove(
