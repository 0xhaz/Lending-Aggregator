...
contract AccessControlUnitTests is MockingSetup {
    ...
    function setUp() public {
        ...
        mockPriceFeed = new MockChainlinkPriceFeed("ETH / USD", 8, 200000000000);
    ...
    //   FujiOracle access control test

    function test_Try_Foe_Set_Price_Feed(address foe) public {
        vm.assume(foe != address(timelock) && foe != address(0) && foe != address(this) && foe != address(chief));
        MockChainlinkPriceFeed maliciousPriceFeed = new MockChainlinkPriceFeed("FakeETH / USD", 8, 1);
        vm.expectRevert(SystemAccessControl.SystemAccessControl__onlyTimelock_callerIsNotTimelock.selector);
        vm.prank(foe);
        fujiOracle.setPriceFeed(address(asset), address(maliciousPriceFeed));
    }

    //   Addrmapper access contorl tests
    function test_Try_Foe_Set_Mapping(address foe, address keyAddr1, address keyAddr2, address returnedAddr) public {
        vm.assume(foe != address(timelock) && foe != address(0) && foe != address(this) && foe != address(chief));
        vm.expectRevert(SystemAccessControl.SystemAccessControl__onlyTimelock_callerIsNotTimelock.selector);
        vm.prank(foe);
